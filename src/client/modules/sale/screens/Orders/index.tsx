...
const Screen = () => {
  ...
  const [orderPagination, setOrderPagination] = useState({ current: 0, page: 1 });
  const [product1Pagination, setProduct1Pagination] = useState({ current: 0, page: 1 });
  const [product2Pagination, setProduct2Pagination] = useState({ current: 0, page: 1 });
  const [product3Pagination, setProduct3Pagination] = useState({ current: 0, page: 1 });
  const [product4Pagination, setProduct4Pagination] = useState({ current: 0, page: 1 });
  ...
  const getProducts = async () => {
    try {
      let data = [];
      if (product1Pagination['current'] < product1Pagination['page']) {
        const ret = await fetchAPI('GET', {
          path: 'products',
          params: {
            page: product1Pagination['current'] + 1,
            filter: [
              {
                where: {
                  deleted: false,
                  categoryId: 'thly8A3mH',
        ...
        if (ret.res.data.length !== 0) {
          setProduct1Pagination(ret.res.pagination);
          data = [...data, ...ret.res.data];
        }
      }

      if (product2Pagination['current'] < product2Pagination['page']) {
        const ret = await fetchAPI('GET', {
          path: 'products',
          params: {
            page: product2Pagination['current'] + 1,
            filter: [
              {
                where: {
                  deleted: false,
        ...
        if (ret.res.data.length !== 0) {
          setProduct2Pagination(ret.res.pagination);
          data = [...data, ...ret.res.data];
        }
      }

      if (product3Pagination['current'] < product3Pagination['page']) {
        const ret = await fetchAPI('GET', {
          path: 'products',
          params: {
            page: product3Pagination['current'] + 1,
            filter: [
              {
                where: {
                  deleted: false,
                  categoryId: '5SZOh8nJo',
                  // applicationId: config.appId,
                },
              },
            ],
        ...
        if (ret.res.data.length !== 0) {
          setProduct3Pagination(ret.res.pagination);
          data = [...data, ...ret.res.data];
        }
      }
      if (product4Pagination['current'] < product4Pagination['page']) {
        const ret = await fetchAPI('GET', {
          path: 'products',
          params: {
            page: product4Pagination['current'] + 1,
            filter: [
              {
                where: {
                  deleted: false,
        ...
        if (ret.res.data.length !== 0) {
          setProduct4Pagination(ret.res.pagination);
          data = [...data, ...ret.res.data];
        }
      }
  ...
  const getOrders = async (reset = false) => {
    try {
      if (orderPagination.current < orderPagination.page || reset) {
        if (reset) {
        ...
        const ret = await fetchAPI('GET', {
          path: 'orders',
          params: {
            page: reset ? 1 : orderPagination.current + 1,
            filter: [
              {
                where: {
                  deleted: false,
        ...
        if (ret.res.data !== undefined) {
          ...
          setOrderPagination(ret.res.pagination);
  ...
  const getContracts = async (orderId: string) => {
    if (orderId !== undefined && orderId !== '') {
      const order = orders.find((item) => item.id === orderId);
      const wholesalerId = order.wholesalerId !== undefined ? order.wholesalerId.id : '';
      const ret = await fetchAPI('GET', {
        path: 'contracts',
        params: {
          filter: [
  ...
  const handleSelectWholesalers = (value: string) => {
    if (value !== 'all') {
      ...
      form.setFieldsValue({
        wholersalerCode: wholesaler.companyId.code,
        wholesalerName: wholesaler.companyId.name,
      });
    } else {
      form.setFieldsValue({
  ...
  const showPrintOrderModal = async (orderId: string) => {
    ...
    const company = orders.find((item) => item.id === orderId).wholesalerId.companyId;
