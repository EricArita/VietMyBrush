...
const Screen = () => {
  ...
  const [paginationWholesaler, setPaginationWholesaler] = useState({ current: 0, page: 1 });
  ...
  const getWholesalers = async () => {
    try {
      if (paginationWholesaler['current'] < paginationWholesaler['page']) {
        const ret = await fetchAPI('GET', {
          path: 'wholesalers',
          params: {
            page: paginationWholesaler['current'] + 1,
            filter: [
              {
                where: {
                  deleted: false,
