...
import { Card, Button, Table, Popconfirm, Form, message, Spin, Row, Col, Select } from 'antd';
...
const Screen = () => {
  ...
  const columns = [
    {
      ...
      render: (_: any, record: any) => {
        return record.companyId !== undefined && record.companyId.code !== undefined ? record.companyId.code : '';
      },
    },
    {
      title: 'Tên',
      ...
      editable: true,
      render: (_: any, record: any) => {
        return record.companyId !== undefined && record.companyId.name !== undefined ? record.companyId.name : '';
      },
    },
    {
      title: 'Địa chỉ',
      ...
      editable: true,
      render: (_: any, record: any) => {
        return record.companyId !== undefined && record.companyId.address !== undefined ? record.companyId.address : '';
      },
    },
    {
      title: 'Số điện thoại',
      ...
      editable: true,
      render: (_: any, record: any) => {
        return record.companyId !== undefined && record.companyId.phone !== undefined ? record.companyId.phone : '';
      },
    },
    {
      title: 'Email',
      ...
      editable: true,
      render: (_: any, record: any) => {
        return record.companyId !== undefined && record.companyId.email !== undefined ? record.companyId.email : '';
      },
    },
    {
      title: 'Công cụ',
      ...
      render: (_: any, record: any) => {
        ...
        return editable ? (
          <span>
            <a
              href='javascript:;'
              onClick={() => saveEditedWholesaler(record)}
              style={{
                marginRight: 8,
              }}
            >
  ...
  const saveEditedWholesaler = async (record: any) => {
    try {
      ...
      const index = wholesalers.findIndex((wholesaler) => wholesaler.id === record.id);
      if (index !== -1) {
        Object.assign(wholesalers[index].companyId, updatedInfo);
    ...
    try {
      if (pagination.current < pagination.page) {
        ...
        if (pagination.current === 0) {
          ...
          setWholesalerOptions([...wholesalers, ...data]);
  ...
  const handleSelectWholesalers = (value: string) => {
    if (value !== 'all') {
      const Wholesalers = wholesalerOptions.filter((item) => item.id === value);
      setWholesalers(Wholesalers);
    }
    else if (value === 'all') {
      setWholesalers(wholesalerOptions);
    }
  };
    ...
    <Card bordered={false}>
      ...
      <div className='body-content'>
        ...
        <div className={'pagination-area'}>
          <Button type='primary' onClick={() => getWholesalers()}>
