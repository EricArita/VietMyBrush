...
const Screen = () => {
  ...
  const onSubmit = async (values: any) => {
    setIsDisabled(true);
    if (!enterInfoManually) {
      await addWholesaler({ companyId: selectedCompanyId });
    } else {
      ...
      <div>
        ...
        >
          <Row gutter={16}>
            ...
            <Col xs={24} sm={12}>
              <Form.Item name='code' label='Mã số doanh nghiệp' rules={[{ required: true, message: 'Mã không được bỏ trống !' }]}>
                <Input disabled={!enterInfoManually} placeholder='Mã số doanh nghiệp' />
              </Form.Item>
